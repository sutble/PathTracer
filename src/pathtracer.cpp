...
Spectrum PathTracer::estimate_direct_lighting_hemisphere(const Ray& r, const Intersection& isect) {
  ...
  for(int i = 0;i<num_samples;i++){
	  ...
	  Ray r = Ray(hit_p + EPS_D*wi_world,wi_world);
	  if(bvh->intersect(r,&isect)){
		  N = N+1;
		  Spectrum radiance = isect.bsdf->get_emission();
		  Spectrum irradiance = isect.bsdf->f(w_out,wi)*wi.z*radiance;
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  //NEED TO NORMALIZE X AND Y HERE
...
//  std::cout<<"x"<<dx<<"\n";
//  std::cout<<"y"<<dy<<"\n";
//  std::cout<<"w"<<frameBuffer.w<<"\n";
//  std::cout<<"h"<<frameBuffer.h<<"\n";
//  std::cout<<"normalized x"<<dx/frameBuffer.w<<"\n";
//  std::cout<<"normalizes y"<<dy/frameBuffer.h<<"\n";



  if(ns_aa == 1){
	  dx = (dx+0.5)/frameBuffer.w;
	  dy = (dy+0.5)/frameBuffer.h;
  ...
  for(int i = 0; i < num_samples; i++){
	  Vector2D ru_sample = gridSampler->get_sample();
	  dx = (dx+ru_sample.x)/frameBuffer.w;
	  dy = (dy+ru_sample.y)/frameBuffer.h;
	  Ray r = camera->generate_ray(dx,dy);
	  total = total + est_radiance_global_illumination(r);
  }
