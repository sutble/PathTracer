...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // 
	...
	//std::cout<<"hFov"<<hFov<<"\n";
	//std::cout<<"vFov"<<vFov<<"\n";
	//std::cout<<"2tr hFov"<<(.99-0)*2*tan(radians(hFov) * .5) - tan(radians(hFov) * .5)<<"\n";
    //std::cout<<"2tr vFov"<<2*tan(radians(vFov) * .5)<<"\n";

	double m_x = (x-0)*(2*tan(radians(hFov) * .5)) - tan(radians(hFov) * .5);
	double m_y = (y-0)*(2*tan(radians(vFov) * .5)) - tan(radians(vFov) * .5);
	//std::cout<<"mx"<<m_x<<"\n";
//	std::cout<<"my"<<m_y<<"\n";
	Vector3D d(m_x,m_y,-1);
